...
def etextno_to_uri_subdirectory(etextno):
    """
...
def format_download_uri(etextno, mirror=None):
    ...
    for extension in extensions:
        path = etextno_to_uri_subdirectory(etextno)
        ...
        if response.ok:
            return uri
    raise requests.exceptions.HTTPError('Failed to find {0} on {1}.'.format(etextno, uri_root))
