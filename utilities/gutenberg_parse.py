...
def get_rdf_data():
    ...
    if not os.path.exists(RDF_FILES):
        _, _ = urllib.request.urlretrieve(RDF_URL, RDF_FILES)
    with tarfile.open(RDF_FILES) as archive:
        for tarinfo in archive:
            yield ElementTree.parse(archive.extractfile(tarinfo))


def parse_metadata(ebook):
    ...
    if creator is not None:
        ...
        if name is not None:
            result['author'] = safe_str(name.text, encoding='utf-8')
    ...
    if title is not None:
        result['title'] = fix_subtitiles(
            safe_str(title.text, encoding='utf-8'))
    # subject lists
...
def fix_subtitiles(title):
    ...
    > fix_subtitiles(u'First Across ...\r\nThe Story of ... \r\n'
    ... 'Being an investigation into ...')
    u'First Across ...: The Story of ...; Being an investigation into ...'"""
    tmp = LINEBREAK_RE.sub(': ', title, 1)
    return LINEBREAK_RE.sub('; ', tmp)


def safe_str(arg, *args, **kwargs):
